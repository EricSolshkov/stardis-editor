...
class SceneTreePanel(QTreeWidget):
    ...
    visibility_changed = pyqtSignal(str, bool)   # body name, visible
...
|---------|-----------|------------|------------|
| 树中单击 Body | 高亮该节点 | 高亮该几何体，其余半透明 | 显示体积属性 |
| 树中单击 Zone | 高亮该节点 | 高亮该区域，其余半透明 | 显示边界参数 |
| 树中勾选可见性 | 复选框状态更新 | 对应 actor 显示/隐藏 | — |
| 3D 中单击几何体 | 自动展开并选中对应节点 | 高亮，其余半透明 | 显示体积属性 |
...
Body "FOAM" 的渲染组件:
  ...
  │       └── 属性: 默认不透明
  │
  └── zone_highlight_actor (区域高亮用)
      ├── vtkExtractCells → 仅含选中区域的 cell
      └── vtkActor (加亮颜色, 线框叠加)

选中效果: 非选中几何体自动设为半透明 (α=0.2)，选中几何体保持不透明。
无需额外 outline_actor。
